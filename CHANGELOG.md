...
### Added
- `wdotool_core::recorder` module behind a new `recorder` Cargo feature. Captures user input from one of three sources — the XDG RemoteDesktop portal (libei in receiver mode), `/dev/input/event*` via evdev, or a deterministic script for tests — and exposes events as a `Stream` of `RecEvent`s. Public surface: `start(RecorderConfig) -> RecorderSession`, `RecorderSession::events() -> impl Stream<Item = RecEvent>`, and `RecorderSession::stop() -> Vec<RecEvent>` for a one-shot capture-and-collect flow. `BackendChoice::Auto` cascades portal then evdev (no fall-through to simulated; fake events that look real are worse than a clear error). Pure input — no window-focus or lifecycle frames; consumers that want those compose their own streams. Capture logic was lifted from wflow's `recorder.rs` (~1100 lines becoming a substrate library) per the design proposal at `docs/design/recorder-migration.md`. PR-2 (a `wdotool record` CLI on top) lands separately; PR-3 in wflow swaps wflow's bundled recorder over to consume this module.
...
### Fixed
- wlroots: chord keys now actually hold modifiers. The wlroots backend pressed modifier keycodes (Control_L, Super_L, …) via `zwp_virtual_keyboard.key` but never called the protocol's `modifiers()` request. Hyprland and other wlroots compositors do not auto-derive the seat's modifier state from virtual-keyboard key events — they require an explicit `modifiers()` update. Fix tracks the currently-depressed XKB modifier bitmask in the worker thread, maps named modifier keysyms (Shift_L/R, Control_L/R, Alt/Meta L/R, Super_L/R, Caps_Lock, ISO_Level3_Shift) to their xkb mod indices via `keymap.mod_get_index()`, and sends `vk.modifiers()` before each modifier press / after each modifier release. Symptom that motivated the fix: `wflow` users running a recorded `ctrl+l → type "hello world"` workflow saw the bare `l` followed by literal `hello world` typed into whatever was focused, because Ctrl was never advertised as held during the leaf key press.
