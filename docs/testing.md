...
This layer doesn't catch bugs in the actual backends. Whether the wlr-protocols backend correctly translates `Press(Control_L)` into a real keyboard event the compositor delivers to the focused window is a question for the next layer.
...
The first run of the round-trip suite caught a real wdotool bug. Without a roundtrip after each input event, the wlr-protocols backend would return from `do_mouse_move` / `do_key` / etc. immediately after `flush()`, the wdotool process would exit, the virtual_keyboard / virtual_pointer would be destroyed, and any in-flight events the compositor hadn't processed yet would be silently dropped. The fix is in `wdotool-core/src/backend/wlr_protocols.rs`: each pointer / keyboard command in the worker loop now does a `queue.roundtrip()` before sending its reply. Layer 2 couldn't have caught this because the mock backend isn't a real Wayland connection.

Eight tests in the suite are `#[ignore]`d. Initial diagnosis was a timing race (observer's `get_pointer` reaching the compositor after `virtual_pointer.motion_absolute` had already been processed); follow-up with `wdotool prime` holding the seat cap up continuously proved the race wasn't the issue. The observer is bound before motion arrives, but the compositor still doesn't fire `wl_pointer.enter` or `wl_pointer.motion`. `swaymsg -t get_seats` reports `capabilities: 0` (no real input devices) even when the wl_seat protocol shows 0x3 to clients, which says the cursor pipeline gates on real input devices, not the virtual ones added via `wlr_virtual_pointer_v1` in headless mode.

Tried three other wlroots compositors hoping it was a sway-specific quirk. Weston is out (the Arch package doesn't ship `zwlr_virtual_pointer_v1`, so wdotool can't even initialize against it). labwc and river both show the exact same shape: caps up, keyboard events work, no pointer events. The gate is in wlroots itself, not in any individual compositor's logic.

What would actually un-ignore these: a non-wlroots target (mutter / kwin via libei in CI), patching wlroots, or a custom test compositor on smithay-rs. None are tractable in the current scope. Real-desktop pointer behavior is covered by the pre-release manual matrix in `docs/verification/`; Layer 2 pins the CLI-to-backend dispatch for every pointer command.
